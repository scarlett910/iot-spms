...
export default function OperatorLayout({
  ...
  const roleDisplay = isAdmin ? "Quản trị viên" : "Nhân viên"
  ...
  const navItems = [
    ...
    { label:"Danh sách vé tạm", href:"/ticket/new"         },
    { label:"Lượt vào/ra",      href:"/sessions"           },
  ...
  if (isMobile) return (
    <>
      ...
      <div suppressHydrationWarning style={{ background:"#F8FAFC", minHeight:"100vh", fontFamily:"'Inter',sans-serif" }}>
        <nav style={{ background:"#003289", height:50, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 16px" }}>
        ...
        <div style={{ padding:"16px 16px 32px" }}>
          ...
          {(title || headerRight) && (
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
              <div>
                {title    && <p style={{ fontWeight:900, fontSize:18, color:"#003289" }}>{title}</p>}
                {subtitle && <p style={{ fontSize:11, color:"#555", marginTop:2 }}>{subtitle}</p>}
              </div>
              {headerRight}
            </div>
          )}

          {/* Mobile nav — chỉ hiện ở trang tổng quan */}
          {activePage === "Tổng quan" && (
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:16 }}>
              {navItems.filter(i => i.label !== "Tổng quan").map(m => (
                <div key={m.label} onClick={() => router.push(m.href)} style={{
                  background:"white", borderRadius:12, padding:"12px 16px",
                  cursor:"pointer", border:"1px solid #e2e8f0",
                  fontWeight:600, fontSize:13, color:"#003289",
                  ...
                  <svg width="14" height="14" viewBox="0 0 18 18" fill="none">
                    <path d="M5 9h8M9 5l4 4-4 4" stroke="#003289" strokeWidth="2" strokeLinecap="round"/>
                  </svg>
                </div>
              ))}
            </div>
    ...
    <>
      ...
      <div suppressHydrationWarning style={{ display:"flex", height:"100vh", background:"#F8FAFC", fontFamily:"'Inter',sans-serif", overflow:"hidden" }}>
        ...
        <div style={{ width:220, background:"#0B4265", display:"flex", flexDirection:"column", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 20px", borderBottom:"1px solid rgba(255,255,255,0.1)" }}>
            ...
            <div>
              ...
              <p style={{ color:"rgba(181,212,244,0.7)", fontSize:10 }}>{roleDisplay}</p>
            </div>
          </div>
            ...
            {navItems.map(item => {
              ...
              return (
                <div key={item.label} onClick={() => router.push(item.href)} style={{
                  ...
                  background: active ? "rgba(217,217,217,0.16)" : "transparent",
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          ...
          {rightPanel && (
            <div style={{ width:220, background:"white", borderLeft:"1px solid #e2e8f0", overflowY:"auto", flexShrink:0 }}>
              {rightPanel}
            </div>
          )}
        </div>
